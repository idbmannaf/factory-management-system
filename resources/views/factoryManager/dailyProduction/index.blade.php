        ...
        <div class="container">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    auth()->user()->selectRole('factory_manager')->hasPermission('production_add'))
                ...
                <div class="card">
                    <div class="card-header bg-blue">
                        {{ucfirst($type)}} Production History
                    </div>
                    <div class="card-body ">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="btn-group btn-sm">
                                                @if (auth()->user()->selectRole('factory_manager')->permissionCount() < 1 || auth()->user()->selectRole('factory_manager')->hasPermission('production_edit'))
                                                <a href="{{route('factory.editDailyProduction',['production'=>$dp->id])}}"  class="btn btn-info btn-xs">Edit</a>
                                                @endif

                                                @if (auth()->user()->selectRole('factory_manager')->permissionCount() < 1 || auth()->user()->selectRole('factory_manager')->hasPermission('production_delete'))
                                                ...
                                                @if (auth()->user()->selectRole('factory_manager')->permissionCount() < 1 || auth()->user()->selectRole('factory_manager')->hasPermission('production_approved'))
                                                <a href="{{route('admin.updateDailyProductionStatus',['production'=>$dp->id,'status'=>'approved'])}}"  class="btn btn-info btn-xs">Approve</a>

                                                @endif

                                                @if (auth()->user()->selectRole('factory_manager')->permissionCount() < 1 || auth()->user()->selectRole('factory_manager')->hasPermission('production_rejected'))
