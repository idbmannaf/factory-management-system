...
$total_price = 0;
        ?>
            ...
            @if (($fb=$sampleItem->raw->firstBatch()) && ($sampleItem->raw->firstBatch()->total_quantity < $multiply))
                ...
                {{-- 2 I --}}
                @if (($sb =$sampleItem->raw->secondBatch()) && ($sampleItem->raw->secondBatch()->total_quantity <
                    $newQty)) <tr class="bg-danger">
                    <td>{{$sampleItem->raw? $sampleItem->raw->name : ''}}({{$sampleItem->raw? $sampleItem->raw->type :
                        ''}})
                    </td>
                    ...
                    {{-- 3 I --}}
                    @if (($tb= $sampleItem->raw->thirdBatch()) && ($sampleItem->raw->thirdBatch()->total_quantity <
                        $secondQty)) <tr class="bg-danger">
                        <td>{{$sampleItem->raw? $sampleItem->raw->name : ''}}({{$sampleItem->raw? $sampleItem->raw->type
                            : ''}})
                        </td>
                    ...
                    $secondQty= $newQty - $tb->total_quantity;
                        ...
                        <tr>
                            <td>{{$sampleItem->raw? $sampleItem->raw->name : ''}}({{$sampleItem->raw?
                                $sampleItem->raw->type : ''}})
                            </td>
                            <td>{{$tb->total_quantity}}</td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>{{$fb->unit_price}}</td>
                            <td>{{$fb->unit_price * $multiply}}</td>
                            <input type="hidden" name="stock[]" value="{{$fb->id}}">
                            <input type="hidden" name="quantity[]" value="{{$multiply}}">
                            <?php
                    $total_unit_price += $fb->unit_price;
                    $total_price += $fb->unit_price * $multiply;
                    ?>
                        </tr>
                        @endif
    ...
    <table class="table table-hovered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="number" name="unit_price" value="{{round($total_unit_price)}}">
                </td>
                <td>
